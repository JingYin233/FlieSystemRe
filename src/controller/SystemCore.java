...
public class SystemCore {
	...
	private int userDatabaseStartBlock = 600; // 用户数据库的起始块ID
	...
	public SystemCore() {
		...
		this.userManager = new UserManager(this.diskManager, userDatabaseStartBlock);
	...
	public String getFileInfo(FCB fcb) {
		...
		result += ("文件名： " + fcb.filename + "\n");
		result += ("类型： "
				+ (fcb.type == FILE_TYPE.FILE ? "文件" : "目录") + "\n");
		result += ("路径： " + this.getCurrentPath() + "\n");
		result += ("创建时间： " + fcb.createdDate + "\n");
		result += ("修改时间： " + fcb.updatedDate+"\n");
		result += ("用户权限： " + fcb.getAuthority()+"\n");
		result +=("创建者账号： "+fcb.CreateName);
	...
	public boolean createDir(String filename) {
		...
		if (fcbBlockStart != 0 && fileDataBlockStart != 0) {
			...
			// 更新新建文件的FCB
			this.updateFCBCreateName(dirFcb);
	...
	public void updateFCB(FCB fcb) {
		...
		// 为创建者账号赋值
		//fcb.CreateName = userManager.getCurrentUser().getUsername();
	 ...
	 * @param fcb
	 */
	public void updateFCBCreateName(FCB fcb) {
		// 为创建者账号赋值
		fcb.CreateName = userManager.getCurrentUser().getUsername();
	...
	public FCB getNewFCB(FCB fcb) {
		Gson gson = new Gson();
		// 读取根目录FCB
		ByteBuffer currentDirFCBBuffer = this.diskManager.io.readDisk(fcb.blockId, fcb.size);
		String currentDirFCBJSON = new String(currentDirFCBBuffer.array(),
				Config.CHARSET);
		FCB fcb1 = gson.fromJson(currentDirFCBJSON, FCB.class);
		return fcb1;
	}
